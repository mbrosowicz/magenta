...
FULL_PIANO_PITCH_CLASSES = [
    [p] for p in  # pylint:disable=g-complex-comprehension
    list(range(PIANO_MIN_MIDI_PITCH, (PIANO_MAX_MIDI_PITCH + 1)))
]

PIANO_PITCH_CLASSES_BY_MAIN_NOTE = [
    [21, 22, 33, 34, 45, 46, 57, 58, 69, 70, 81, 82, 93, 94, 105, 106],
    [23, 35, 47, 59, 71, 83, 95, 107],
    [24, 25, 36, 37, 48, 49, 60, 61, 72, 73, 84, 85, 96, 97, 108],
    [26, 27, 38, 39, 50, 51, 62, 63, 74, 75, 86, 87, 98, 99],
    [28, 40, 52, 64, 76, 88, 100],
    [29, 30, 41, 42, 53, 54, 65, 66, 77, 78, 89, 90, 101, 102],
    [31, 32, 43, 44, 55, 56, 67, 68, 79, 80, 91, 92, 103, 104]
]

PIANO_PITCH_CLASSES_BY_NOTE = [
...
def maybe_sample_items(seq, sample_size, randomize):
    ...
    if randomize:
        indices = set(np.random.choice(
            len(seq), size=sample_size, replace=False))
...
class LegacyEventListOneHotConverter(BaseNoteSequenceConverter):
    ...
    def _dedupe_and_sample(self, all_sliced_lists):
        ...
        sliced_multievent_lists = [zip(*lists)
                                   for lists in zip(*all_sliced_lists)]
    ...
    def _chords_and_keys_to_controls(self, chord_tuples, key_tuples):
        ...
        for ct, kt in itertools.zip_longest(chord_tuples, key_tuples):
            ...
            if ct is not None:
                try:
                    chord_tokens = [
                        self._chord_encoding.encode_event(e) for e in ct]
                    if self.end_token:
    ...
    def from_tensors(self, samples, controls=None):
        ...
        for i, sample in enumerate(samples):
            ...
            if self._steps_per_quarter:
                ...
                sequence = event_list.to_sequence(
                    velocity=OUTPUT_VELOCITY, qpm=qpm)
            else:
            ...
            if self._chord_encoding and controls is not None:
                ...
                chord_times = [
                    step * seconds_per_step for step in event_list.steps]
                chords_lib.add_chords_to_sequence(
                    sequence, chords, chord_times)
...
class DrumsConverter(BaseNoteSequenceConverter):
    ...
    def _to_tensors_fn(self, note_sequence):
        ...
        event_lists, unused_stats = self._drums_extractor_fn(
            quantized_sequence)
        ...
        for t in unique_event_tuples:
            if self._roll_input or self._roll_output:
                if self.end_token is not None:
                    t_roll = list(
                        t) + [(self._pr_encoder_decoder.input_size - 1,)]
                else:
                    t_roll = t
                rolls.append(np.vstack([
                    self._pr_encoder_decoder.events_to_input(
                        t_roll, i).astype(np.bool)
            ...
            if not (self._roll_input and self._roll_output):
                ...
                oh_vecs.append(np_onehot(
                    labels,
                    self._oh_encoder_decoder.num_classes +
                    (self.end_token is not None),
                    np.bool))
    ...
    def from_tensors(self, samples, unused_controls=None):
        ...
        for s in samples:
            ...
            output_sequences.append(
                track.to_sequence(velocity=OUTPUT_VELOCITY))
        return output_sequences
...
class TrioConverter(BaseNoteSequenceConverter):
    ...
    def _to_tensors_fn(self, note_sequence):
        ...
        for note in quantized_sequence.notes:
            ...
            end_bar = int(
                np.ceil(note.quantized_end_step / self._steps_per_bar))
        ...
        for i in (instruments_by_type[self.InstrumentType.MEL] +
            ...
            if tensors.outputs:
                ...
                if encoded_controls is None:
                    ...
                    tf.logging.warning(
                        'Trio controls disagreement between instruments.')
            else:
                coverage[:, i] = False
        ...
        for grp in itertools.product(
            ...
            grp_coverage[:self._gap_bars] = np.any(
                coverage[:self._gap_bars, grp])
            grp_coverage[-self._gap_bars:] = np.any(
                coverage[-self._gap_bars:, grp])
            for j in range(total_bars - self._slice_bars + 1):
                if (np.all(grp_coverage[j:j + self._slice_bars]) and
                    ...
                    seqs.append(np.concatenate(
                        [encoded_instruments[i][start_step:end_step]
                            for i in grp],
                        axis=-1))
                    if encoded_controls is not None:
                        control_seqs.append(
                            encoded_controls[start_step:end_step])
...
class GrooveConverter(BaseNoteSequenceConverter):
    ...
    def __init__(self, split_bars=None, steps_per_quarter=4, quarters_per_bar=4,
        ...
        self._infer_pitch_class_map = _classes_to_map(
            self._infer_pitch_classes)
    ...
    def _get_feature(self, note, feature, step_length=None):
        ...
        if feature == 'hit':
            ...
            if note:
                ...
                # Switch from [-0.5, 0.5] to [-1, 1] for tensors.
                return offset*2
            else:
    ...
    def to_tensors(self, note_sequence):
        ...
        try:
            ...
            if not quantized_sequence.time_signatures:
                quantized_sequence.time_signatures.add(
        ...
        for step in range(max_step):
            ...
            for drum in range(self._num_drums):
                ...
                velocity_vectors[step, drum] = self._get_feature(
                    note, 'velocity')
        ...
        if self._tapify:
            ...
            in_velocities[:, 3] = velocity_vectors[np.arange(
                max_step), argmaxes]
            ...
            for i in range(n_timesteps):
                if self._categorical_outputs:
                    ...
                    if self._split_instruments:
                        ...
                        hits, velocities, offsets = np.split(  # pylint: disable=unbalanced-tuple-unpacking
                            sample[i],
                            [self._num_drums, self._num_drums * \
                ...
                for j in range(len(hits)):
                    ...
                    if hits[j] > 0.5:
                        ...
                        if self._categorical_outputs:
                            ...
                            note.velocity = _zero_one_to_velocity(
                                velocities[j])
...
def convert_to_tensors_op(item_scalar, converter):
    ...
    def _convert_and_pad(item_str):
        item = converter.str_to_item_fn(
            item_str.numpy())  # pylint:disable=not-callable
    ...
    if examples_path:
        ...
        def _tf_midi_to_note_sequence(ex):
            return tf.py_function(
                ...
                [note_seq.midi_to_note_sequence(
                    x.numpy()).SerializeToString()],
                inp=[ex['midi']],
                Tout=tf.string,
